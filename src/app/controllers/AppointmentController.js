...
class AppointmentController {
  ...
  async delete(req, res) {
    ...
    const dateWithSub = subHours(appointment.date, 2); // Remove 2h do horario do agendamento

    // agendamento 13h
    // dateWithSub 11h
    // new Date = 11:25 (hora atual)

    if (isBefore(dateWithSub, new Date())) {
      return res
        .status(401)
        .json({ error: 'You can only cancel appointments 2 hours in advance' });
    } // Se 11h for antes que 11:25 -> não pode desmarcar
